...
if tables:
    ...
    if selected_table:
        ...
        if selected_table == "speed-distribution":
            ...
            intersection_filter = st.text_input(
                "Filter by Intersection (e.g., 'glebe-potomac')", ""
            )

            if intersection_filter:
                filter_params = (
                    f"&filter_col=intersection&filter_val={intersection_filter}"
                )
                st.info(f"Filtering by intersection: {intersection_filter}")

        limit = st.slider("Rows to fetch", 10, 5000, 1000)
        data = fetch_api(
            f"/database/data/{selected_table}?limit={limit}{filter_params}"
        )
        ...
        if data:
            ...
            if not df.empty:
                ...
                for col in df.columns:
                    if "date" in col.lower() or "time" in col.lower():
                        try:
                            ...
                            if (
                                df[col].dtype == "int64"
                                and df[col].mean() > 1000000000000
                            ):
                                df[col] = pd.to_datetime(df[col], unit="ms")
                            # Handle BigInt timestamps (seconds)
                            elif (
                                df[col].dtype == "int64" and df[col].mean() > 1000000000
                            ):
                ...
                categorical_cols = [
                    c for c in categorical_cols if c not in datetime_cols
                ]
                ...
                if viz_type == "Speed Distribution Over Time":
                    if (
                        ...
                        fig = px.bar(
                            ...
                            labels={
                                ...
                                "count": "Vehicle Count",
                                "speed_interval": "Speed Range",
                            },
                        )
                        st.plotly_chart(fig, use_container_width=True)
                    else:
                        st.warning(
                            "Required columns (start_timestamp, speed_interval, count) not found."
                        )
                    ...
                    if len(numeric_cols) >= 2:
                        ...
                        color_col = st.selectbox(
                            "Color (Optional)", ["None"] + categorical_cols
                        )
                    ...
                    if numeric_cols:
                        ...
                        x_col = st.selectbox(
                            "Group By (Optional)", ["None"] + categorical_cols
                        )
                    ...
                    lat_cols = [
                        c for c in df.columns if "lat" in c.lower() or "y" == c.lower()
                    ]
                    lon_cols = [
                        c for c in df.columns if "lon" in c.lower() or "x" == c.lower()
                    ]
