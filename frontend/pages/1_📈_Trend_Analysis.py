...
API_BASE_URL = API_URL.rstrip('/').rsplit('/', 1)[0] if API_URL.endswith('/') else API_URL
...
def get_safety_score_trend(
    ...
    try:
        ...
        response = requests.get(
            f"{API_BASE_URL}/time/range", params=params, timeout=60
        )
        response.raise_for_status()
        return response.json()
    except requests.HTTPError as e:
...
def render_single_time_view(intersection: str, selected_time: datetime, bin_minutes: int):
    ...
    if not data:
        ...
        st.info("💡 Try selecting a different time or intersection. Available data up to November 9, 2025.")
        return
    ...
    st.caption("Different multi-criteria decision making methods used in the calculation")
...
def render_trend_view(intersection: str, start_time: datetime, end_time: datetime, bin_minutes: int):
    ...
    if not data:
        ...
        st.info("💡 Try selecting a different time range or intersection. Available data up to November 9, 2025.")
        return
...
def main():
    ...
    if analysis_mode == "Single Time Point":
        ...
        render_trend_view(selected_intersection, start_datetime, end_datetime, bin_minutes)
