...
\caption{Refined cloud-native system architecture of VTTSI. The left pathway shows the
safety-index pipeline: Streamlit analysis pages call the FastAPI REST endpoints, which
dispatch to the Hybrid MCDM Service and RT-SI Service; both services query the shared
Cloud SQL (PostgreSQL + PostGIS) database. The right pathway shows the SafetyChat
module: the natural-language interface page calls \texttt{POST /api/v1/chat/}, which
routes to a tool-augmented Chat Service that invokes four live-data tools via the
OpenAI GPT-4o function-calling API. Dashed arrows indicate external ETL ingestion
from the VCC/Trino connected-vehicle platform and VDOT historical crash records.}
