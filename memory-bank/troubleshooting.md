...
### Problem: Sensitivity Analysis Timeout (>30s)

**Symptom:**
Frontend shows "AxiosError: timeout" or API returns 504 Gateway Timeout when running analysis for long date ranges (e.g., > 20 days).

**Cause:**
Calculating safety indices for 50-100 perturbation scenarios over thousands of time bins resulted in excessive database queries (N\*M problem).

**Solution:**
The system has been optimized to use **In-Memory Calculation**.

1. Ensure you are using the latest version of the backend.
2. If the issue persists for extremely long ranges (> 90 days), try breaking the analysis into smaller chunks (e.g., month by month).

**Verification:**
Check API logs to see if `calculate_rt_si_from_data` is being used instead of repeated database calls.
