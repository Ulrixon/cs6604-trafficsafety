% ============================================================
%  VTTSI Refined System Architecture (with SafetyChat Module)
...
\begin{tikzpicture}[
    ...
    page/.style    ={rectangle, draw, rounded corners=3pt, fill=blue!9,
                     minimum width=2.5cm, minimum height=1.35cm,
...
% ── TIER 1 : Streamlit Frontend pages ────────────────────────────────────────
\node[page, below=1.3cm of user, xshift=-5.3cm] (p0)
    {\textbf{Dashboard}\\Geospatial Map\\$\alpha$-Blend};
...
\node[chatpage, right=0.6cm of p3] (p4)
    {\textbf{SafetyChat}\\NL Query\\Interface};

\node[grpbox, fit=(p0)(p1)(p2)(p3)(p4),
      label={[font=\small\bfseries, fill=white, inner sep=2pt,
              rounded corners=2pt]above:{Streamlit Frontend (Google Cloud Run)}}]
      (fe_box) {};
...
\draw[arr] (p4.south) -- node[right, font=\scriptsize]{HTTPS / REST}
    (api_chat.north);
